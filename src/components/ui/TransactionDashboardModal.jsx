...
function makeData(title) {
  ...
  const stats = [
    { label: 'Total Transactions', value: base.toLocaleString(),                    bdtCls: 'kpi-bdt-blue',   valCls: 'kpi-val-blue'   },
    { label: 'Unique IPs',         value: Math.round(base * 0.12).toLocaleString(), bdtCls: 'kpi-bdt-cyan',   valCls: 'kpi-val-cyan'   },
    { label: 'Unique Networks',    value: '18',                                      bdtCls: 'kpi-bdt-violet', valCls: 'kpi-val-violet' },
    { label: 'Avg Score',          value: isExcluded ? '3.2' : '8.7',               bdtCls: 'kpi-bdt-green',  valCls: 'kpi-val-green'  },
  ];
      ...
      <div className="tdm-modal">
        ...
        <div className={`tdm-header${isExcluded ? ' excluded' : ''}`}>
          <div className="tdm-header-left">
            ...
            <div>
              ...
              <div className="tdm-header-subtitle">{title} — filtered overview</div>
            </div>
          </div>
        ...
        <div className="tdm-body">
          ...
          <div className="tdm-card">
            <SectionHead colorClass="tdd-bar-blue">Transaction Trend — Last 14 Days</SectionHead>
            ...
            <div className="tdm-legend">
              {[['Visits','legend-dot-blue'],['Clicks','legend-dot-green'],['Blocks','legend-dot-rose']].map(([l, cls]) => (
                <div key={l} className="tdm-legend-item">
                  <div className={`tdm-legend-dot ${cls}`} />{l}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="tdm-charts-row">
            <div className="tdm-card">
              <SectionHead colorClass="tdd-bar-amber">Transactions by Hour</SectionHead>
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={data.byHour} margin={{ top: 0, right: 8, bottom: 0, left: -20 }}>
                  <XAxis dataKey="h" tick={{ fontSize: 8, fill: '#cbd5e1' }} axisLine={false} tickLine={false} interval={2} />
                  <YAxis tick={{ fontSize: 9, fill: '#cbd5e1' }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={{ fontSize: 11, borderRadius: 8 }} />
                  <Bar dataKey="count" name="Transactions" radius={[3, 3, 0, 0]}>
                    {data.byHour.map((_, i) => (
                      <Cell key={i} fill={i >= 6 && i <= 20 ? BLUE : VIOLET} fillOpacity={0.8} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
                ...
                <div className="tdm-network-legend">
                  {data.byNetwork.map((n, i) => (
                    <div key={n.name} className="tdm-network-item">
                      ...
                      <span className={`tdm-network-pct pal-col-${i}`}>{n.value}%</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* ── Block reasons bar ── */}
          <div className="tdm-card">
            <SectionHead colorClass="tdd-bar-rose">Block Reason Distribution</SectionHead>
            <div className="tdm-reason-grid">
              {data.byReason.map((r, i) => (
                <div key={r.name} className={`tdm-reason-card reason-card-${i}`}>
                  <div className={`tdm-reason-pct pal-col-${i}`}>{r.value}%</div>
                  <div className="tdm-reason-badge-wrap">
                    <span className={`tdm-reason-badge pal-bg-${i}`}>{r.name}</span>
                  </div>
                  <div className="tdm-reason-bar-bg">
                    {/* width is runtime data — only this single CSS custom property is unavoidable */}
                    <div className={`tdm-reason-bar-fill pal-bg-${i}`} style={{ '--fill': `${r.value}%` }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="tdm-footer">
          <button onClick={onClose} className="tdm-btn-close">Close</button>
        </div>
      </div>
    </>
  );
